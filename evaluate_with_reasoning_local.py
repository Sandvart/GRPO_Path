...
class ModelEvaluator:
    ...
    def __init__(self, reasoning_model_path: str,
                 answer_api_key: str, answer_base_url: str,
                 answer_model: str = "qwen-turbo",
                 device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        # 加载本地推理指示器生成模型
        print(f"正在加载推理指示器生成模型: {reasoning_model_path}")
        print(f"使用设备: {device}")
        
        self.device = device
        self.reasoning_tokenizer = AutoTokenizer.from_pretrained(reasoning_model_path, trust_remote_code=True)
        ...
        if device == "cpu":
            self.reasoning_model = self.reasoning_model.to(device)
    ...
    def generate_reasoning_indicator(self, question: str, instruction: str) -> Dict:
        ...
        try:
            ...
            # 编码输入
            model_inputs = self.reasoning_tokenizer([text], return_tensors="pt").to(self.device)
...
def main():
    """主函数"""
    # 配置参数 - 推理指示器生成模型(本地模型路径)
    REASONING_MODEL_PATH = "/home/shahy/GRPO_test/outputs/GRPO_Path_Reasoning_Indicator_webqsp_1.5b/final_model"
    ...
    NUM_SAMPLES = 100
    OUTPUT_FILE = "evaluation_results_with_reasoning_local.json"
